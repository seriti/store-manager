...
use App\Store\ReceiveWizard;

class ReceiveWizardController
{
    protected $container;
    ...
    public function __invoke($request, $response, $args)
    {
        ...
        $user_specific = true;
        $cache_name = 'receive_wizard';
        $cache->setCache($cache_name,$user_specific);

        $wizard_template = new Template(BASE_TEMPLATE);
        
        $wizard = new ReceiveWizard($db,$this->container,$cache,$wizard_template);
        $wizard->setup();        

        $html = $wizard->process();

        $template['html'] = $html;
        $template['title'] = MODULE_LOGO.'Reception wizard';
